...
export default function ServerFilter() {
  ...
  const filterServers = async (greenCheck: boolean, redCheck: boolean) => {
      ...
      const filterData = await axios.post(`http://89.104.65.22:3000/companies/filter`, 
      {
          name: filterName,
          status: filterStatus
      });
        ...
        if (!company.server.length) {
          ...
          await Promise.all(
            company.server.map(async (server) => {
              try {
                const response = await axios.get(`http://89.104.65.22:3000/system/status/server/${server.idServer}`);
                okay++;
