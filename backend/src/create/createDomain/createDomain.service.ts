...
export class DomainService {
  private prisma =  prisma; // Инициализируем Prisma для взаимодействия с базой данных
  constructor(
  ...
  public async getWhoisData(
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(`http://89.104.65.22:3000/whois?domain=${domain}`),
      );
  ...
  public async getSSLabsData(domain: string) {
    try {
      const response = await axios.get(
        `http://89.104.65.22:3000/ssl-labs/analyze/${domain}`,
      );
      return response.data;
    } catch (error) {
      console.error('Ошибка при получении информации о домене:', error);
  ...
  public async getPagesData(
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(`http://89.104.65.22:3000/pages/${domain}/${idApp}`),
      );
      return response.data;
    } catch (error) {
