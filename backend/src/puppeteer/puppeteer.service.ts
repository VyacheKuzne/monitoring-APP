...
export class PuppeteerService {
  ...
  async updatePageData(urls: string[], pageCount: number): Promise<void> {
    ...
    const processUrl = async (url: string): Promise<void> => {
      ...
      try {
        this.logger.log(`В процесссе анализа URL: ${url}`);
        ...
        while (attempts--) {
          try {
            ...
            this.logger.error(
              `Не удалось прочитать страницу, оставшиеся проверки: ${attempts}`,
            );
            if (attempts === 0) {
              this.logger.error(`Попытки закончились. Полный провал`);
        ...
        try {
          if (statusLoadPage !== 'Error') {
            ...
            this.logger.log(`Страница ${url} загрузилась корректно`);
            if (!this.idApp) {
          ...
          this.logger.error(`Ошибка при загрузке страницы: ${error.message}`);
          PageData.title = url;
  ...
  async stopMonitoring(domain: string) {
    this.logger.log(
      `Проверка домейна: ${domain} завершенна. Общее количество страниц после провекри: ${this.PageCount}, Количество страниц с результатом error: ${this.failedPageCount}`,
    );
