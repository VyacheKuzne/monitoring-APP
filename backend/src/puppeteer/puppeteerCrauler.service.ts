...
export class PuppeteerCrauler {
  ...
  async startMonitoring(domain: string) {
    this.logger.log(`Стартуем проверку домена: ${domain}`);
  ...
  async checkSitemap(domain: string) {
    try {
      ...
      if (sitemapLines.length > 0) {
        this.logger.log(`Сайтмап найден, нет необходимости в ручном поиске: ${sitemapLines}`);
        await this.findLinksInSitemap(sitemapLines, domain);
      } else {
        this.logger.log(`Сайтмап не найден или его нет, начинаем ручную проверку`);
        await this.findLinksViaPuppeteer(domain);
      }
    } catch (error) {
      this.logger.error(
        `Failed to fetch robots.txt for ${domain}: ${error.message}`,
      );
      await this.findLinksViaPuppeteer(domain);
    }
  }
  ...
  async findLinksViaPuppeteer(domain: string) {
    ...
    const exploreLinks = async () => {
      ...
      await this.puppeteerService.updatePageData(
