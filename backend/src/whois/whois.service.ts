...
export class WhoisService {
  ...
  private prisma = prisma; // Создаём Prisma клиент
          ...
          return from(
            new Promise<WhoisData>((resolve, reject) => {
              parseString(response.data, (err, result: WhoisResponse) => {
                ...
                const createdDate =
                  whoisRecord?.createdDate?.[0] ||
                  registryData?.createdDate?.[0] ||
                  undefined;
                const updatedDate = whoisRecord?.updatedDate?.[0] || undefined;
                const expiresDate =
                  whoisRecord?.expiresDate?.[0] ||
                  registryData?.expiresDate?.[0] ||
                  undefined;
                const registrarName =
                  whoisRecord?.registrarName?.[0] || undefined;
                const organization =
                  whoisRecord?.registrant?.organization?.[0] || undefined;
  ...
  async createDomain(whoisData: WhoisData): Promise<void> {
    if (whoisData) {
      await this.prisma.domain.upsert({
        ...
        update: {
          registered:
            whoisData.creationDate instanceof Date
              ? whoisData.creationDate.toISOString()
              : whoisData.creationDate,
          updated:
            whoisData.updatingDate instanceof Date
              ? whoisData.updatingDate.toISOString()
              : whoisData.updatingDate,
          expires:
            whoisData.expiresDate instanceof Date
              ? whoisData.expiresDate.toISOString()
      ...
      this.logger.log(
        `Whois data ${whoisData.domainName} recorded successfully`,
      );
