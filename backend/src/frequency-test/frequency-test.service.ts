...
export class FrequencyTestService implements OnModuleInit, OnModuleDestroy {
  ...
  async getAllDomain() {
    try {
      ...
      console.log(
        `Успешно получены домены и идентификаторы приложений: ${domainInfo.map((d) => `${d.idApp} -> ${d.domain?.name ?? 'Нет данных'}`).join(', ')}`,
      );
  ...
  public async updateWhoisData(): Promise<void> {
    ...
    for (const domainData of domain.domainInfo) {
      this.logger.log(
        `🔍 Начало автоматической проверки WHOIS для домена: ${domainData.domain.name}`,
      );
      try {
        const updateResultWhois = await firstValueFrom(
          this.httpService.get(
            `http://89.104.65.22:3000/whois?domain=${domainData.domain.name}`,
          ),
        );
        this.logger.log(
          `✅ Завершена автоматическая проверка WHOIS для домена ${domainData.domain.name}: ${JSON.stringify(updateResultWhois.data)}`,
        );
      } catch (error) {
        this.logger.error(
          `❌ Ошибка при получении информации WHOIS для домена: ${domainData.domain.name}`,
          error,
        );
      }
    }
  ...
  public async updateSSLabsData(): Promise<void> {
    ...
    for (const domainData of domain.domainInfo) {
      this.logger.log(
        `🔍 Начало автоматической проверки SSL для домена: ${domainData.domain.name}`,
      );
      try {
        await firstValueFrom(
          this.httpService.get(
            `http://89.104.65.22:3000/ssl-labs/analyze/${domainData.domain.name}`,
          ),
        );
        this.logger.log(
          `✅ Завершена автоматическая проверка SSL для домена ${domainData.domain.name}`,
        );
      } catch (error) {
        this.logger.error(
          `❌ Ошибка при получении информации SSL для домена: ${domainData.domain.name}`,
          error,
        );
      }
    }
  ...
  public async updatePagesData(): Promise<void> {
    ...
    for (const domainData of domain.domainInfo) {
      this.logger.log(
        `🔍 Начало автоматической проверки страниц для домена: ${domainData.domain.name}`,
      );
      ...
      try {
        ...
        const updateResultPages = await firstValueFrom(
          this.httpService.get(
            `http://89.104.65.22:3000/pages/${domainData.domain.name}/${domainData.idApp}`,
          ),
        );
        ...
        if (updateResultPages.data?.status === 'in_progress') {
          this.logger.warn(
            `⚠️ Мониторинг уже выполняется для домена: ${domainData.domain.name}, пропускаем...`,
          );
        } else {
          this.logger.log(
            `✅ Завершена автоматическая проверка страниц для домена ${domainData.domain.name}: ${JSON.stringify(updateResultPages.data)}`,
          );
        }
      } catch (error) {
        this.logger.error(
          `❌ Ошибка при получении информации о страницах для домена: ${domainData.domain.name}`,
          error,
        );
      }
    }
