...
const OperationStatusChart: React.FC<Props> = ({ allData }) => {
  ...
  allData.forEach((data) => {
    ...
    const hourDiff = Math.floor(
      (now.getTime() - dataDate.getTime()) / (60 * 60 * 1000),
    );
    ...
    <div className="flex flex-col gap-[10px] w-full mt-[27px]">
      <div className="flex gap-[8px] items-center">
        {[...Array(totalGroups)].map((_, groupIndex) => {
          ...
          const endHourOffset = Math.min(
            startHourOffset + groupSize + 1,
            hours + 1,
          );
            ...
            >
              <div className="relative flex justify-center">
                <div className="absolute top-[-27px] text-[12px]">
                  {getHourLabel(startHourOffset + 1)} -{" "}
                  {getHourLabel(endHourOffset)}
                </div>
