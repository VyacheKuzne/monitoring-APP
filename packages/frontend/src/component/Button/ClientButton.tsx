...
export default function ClientButton({ companiesData }: ModalBlockProps) {
    ...
    const allowedLocation = allowedPaths.some((path) => location.pathname.startsWith(path));
    ...
    const initState = () => {
        if (allowedLocation) { // Если путь разрешённый, то открываем панель
            setIsOpen(true);
        }
    }
                    ...
                    <ul className='text-left border-l-[3px] border-gray-500 px-[3%]'>
                        {
                            clients.map((client, index) =>(
                                <li className='relative flex items-center px-[5%] py-[3%]' key={index}>
                                    {client.idCompany === Number(idCompany) ? <div className='absolute left-[-14px] min-w-[11px] w-[11px] min-h-[11px] h-[11px] bg-green-500 border-[2px] border-white rounded-full' /> : null}
