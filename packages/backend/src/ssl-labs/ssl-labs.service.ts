    ...
    return this.analyze(host).pipe(
      switchMap((initialData) => {
        if (initialData.status === 'READY') {
          ...
          return throwError(
            () => new Error(`SSL Labs analysis failed for ${host}`),
          );
        } else {
          ...
          return interval(5000).pipe(
            ...
            takeWhile(
              (data) => data.status !== 'READY' && data.status !== 'ERROR',
              true,
            ),
            last(),
            tap((finalData) => {
            ...
            catchError((err) => {
              this.logger.error(
                `Error during polling for ${host}: ${err.message}`,
              );
              return throwError(
                () =>
                  new Error(`Error during polling for ${host}: ${err.message}`),
              );
            }),
          );
        }
      ...
      switchMap((data) =>
        from(this.transformToSslResult(data)).pipe(
          switchMap((sslData) => from(this.recordSSL(sslData))),
          map(() => data), // Возвращаем исходные данные
        ),
      ),
    );
  }
      ...
      return {
        ...
        namePublisher:
          cert.issuerSubject
            ...
            ?.replace('CN=', '') || 'Unknown',
        registered: cert.notBefore
          ? new Date(cert.notBefore)
          : new Date(data.startTime),
        expires: cert.notAfter
          ? new Date(cert.notAfter)
          : new Date(data.testTime),
