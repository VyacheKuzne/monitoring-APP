   ...
   async getResourceStatus(page: puppeteer.Page) {
    try {
        ...
        page.on('response', (response) => {
          ...
          if (contentType?.includes('stylesheet') || contentType?.includes('text/css')) {
                if (status >= 200 && status <= 304) {
                    responses.add('style');
                    this.logger.debug(`✅ CSS loaded: ${url} (fromCache: ${fromCache})`);
                } else {
                    failedCssRequests.push(url);
                    this.logger.error(`❌ CSS failed to load: ${url} with status: ${status}`);
                }
        ...
        this.logger.log(`No CSS content-type found for: ${url}. Content-Type: ${contentType}`);
        }
        ...
        page.on('requestfailed', (request) => {
            ...
            this.logger.error(`❌ Request failed: ${url} with error: ${request.failure()?.errorText}`);
            failedRequests.push(url);
        ...
        if (failedRequests.length > 0) {
            this.logger.error(`🚨 Failed resource requests on page ${page.url()}: ${failedRequests.join(', ')}`);
            this.logger.error(`📌 Detailed failed requests on page ${page.url()}: ${failedRequestDetails.join('; ')}`);
        }

        return {
            allLoaded: networkStatus.styleStatus === "Loaded" &&
            ...
            finalStatus: (networkStatus.styleStatus === "Loaded" &&
                          networkStatus.mediaStatus === "Loaded" &&
                          networkStatus.scriptStatus === "Loaded")
