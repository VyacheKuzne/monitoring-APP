...
export class DomainController {
  ...
  async createDomain(@Body() Body: { name: string; appName: string; idCompany: number; idServer: number }) {
    ...
    this.progressGateway.server.emit('progress', { progress: 0, message: 'Начало создания домена' });

    try {
      // Выполняем создание домена и приложения
      const createdApp = await this.domainService.createDomainAndLinkDomain(
        Body.name, Body.appName, Body.idCompany, Body.idServer
      );

      // Если процесс завершился успешно
      this.progressGateway.server.emit('progress', { progress: 100, message: 'Домен и приложение успешно созданы', app: createdApp });

      return createdApp;  // Возвращаем результат
    } catch (error) {
      // В случае ошибки, отправляем информацию о сбое
      this.progressGateway.server.emit('progress', { progress: 100, message: `Ошибка: ${error.message}` });
